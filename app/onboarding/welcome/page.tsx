...
/* ─── Particles generated in useState lazy initialization (client only) ──────── */
...
function Heart3D({ progress, accentFrom, accentTo, glowColor }: {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 140, height: 130 }}>
      ...
      <svg
        width="120" height="110" viewBox="0 0 120 110"
      ...
      >
        ...
        <path
          ...
          fill="none" stroke={`${accentFrom}30`} strokeWidth="2"
        />
        <rect
          x="0" y={110 - fill * 110} width="120" height="110"
          fill="url(#heartGradFill)" clipPath="url(#heartClip)"
          style={{ transition: "y 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}
        />
        <path
          d="M60 95 C20 70 5 50 5 32 C5 15 18 5 32 5 C42 5 52 11 60 20 C68 11 78 5 88 5 C102 5 115 15 115 32 C115 50 100 70 60 95 Z"
          fill="url(#heartShine)" clipPath="url(#heartClip)"
        />
        <ellipse
          cx="42" cy="28" rx="10" ry="7" fill="white"
          opacity={0.08 + fill * 0.06} transform="rotate(-20 42 28)"
        />
      </svg>
...
function CoupleApproach({ progress }: { progress: number }) {
  ...
  return (
    <div className="relative flex items-end justify-center gap-0" style={{ height: 64, opacity }}>
      <motion.div animate={{ x: leftX }} transition={{ duration: 0.6, ease: "easeOut" }}>
      ...
      <motion.div animate={{ x: rightX }} transition={{ duration: 0.6, ease: "easeOut" }}>
        <svg width="28" height="52" viewBox="0 0 28 52" fill="rgba(255,255,255,0.18)">
          <circle cx="14" cy="10" r="7" />
          <path d="M4 52 C4 36 8 28 14 28 C20 28 24 36 24 52 Z" />
        </svg>
...
export default function WelcomePage() {
  ...
  const { t } = useI18n();
  const choices = useState(() => getUserChoices())[0];
  ...
  /*
   * Map SparkTheme properties to the values Heart3D needs.
   * SparkTheme uses: theme.accent (single color), theme.secondary,
   * theme.accentGlow, theme.accentRgb, theme.btnPrimaryBg
   * It does NOT have accentFrom / accentTo — we derive them safely.
   */
  const heartAccentFrom: string = (theme as unknown as Record<string, unknown>).accentFrom as string
    ?? (theme as unknown as Record<string, unknown>).accent as string
    ?? "#f43f5e";

  const heartAccentTo: string = (theme as unknown as Record<string, unknown>).accentTo as string
    ?? (theme as unknown as Record<string, unknown>).secondary as string
    ?? "#a855f7";

  const heartGlow: string = (theme as unknown as Record<string, unknown>).accentGlow as string
    ?? (theme as unknown as Record<string, unknown>).glowHero as string
    ?? "rgba(244,63,94,0.35)";
  ...
  useEffect(() => {
    if (!done) return;
    const timer = setTimeout(() => router.push("/home"), 1400);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: [
            `radial-gradient(ellipse 90% 55% at 50% 35%, ${heartGlow} 0%, transparent 70%)`,
            `radial-gradient(ellipse 70% 45% at 50% 95%, ${(theme as unknown as Record<string,unknown>).secondaryGlow as string ?? "rgba(168,85,247,0.15)"} 0%, transparent 70%)`,
          ].join(", "),
        }}
      />

      {/* Particles — client only via useMemo, no SSR mismatch */}
      {particles.map((p) => (
        <motion.div
          key={p.id}
          ...
          style={{
            ...
            width:  p.size,
            height: p.size,
            background: `rgba(${(theme as unknown as Record<string,unknown>).accentRgb as string ?? "244,63,94"}, 0.25)`,
          }}
      ...
      <div className="relative z-10 w-full max-w-sm px-6 flex flex-col items-center text-center">
        ...
        >
          <Heart3D
            progress={progress}
            accentFrom={heartAccentFrom}
            accentTo={heartAccentTo}
            glowColor={heartGlow}
          />
        </motion.div>
        ...
        <div className="w-full max-w-[260px]">
          <div
              className="w-full h-1.5 rounded-full overflow-hidden mb-2 progress-bar-bg"
              role="progressbar"
              aria-label="Setup progress"
              aria-valuenow={Math.round(progress) as unknown as number}
          ...
          <div className="flex justify-between">
            <span className="text-[11px] tabular-nums font-medium progress-text" style={{ color: theme.textMuted }}>
              {progress}%
            </span>
            <span className="text-[11px] progress-text" style={{ color: theme.textMuted }}>
              {done
